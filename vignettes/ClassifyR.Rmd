...
The type of classifier used can be changed with the *classifier* argument. The default is a random forest, which seamlessly handles categorical and numerical data. A full list of classifiers can be seen by running *?crossValidate*. Similarly, the number of folds and number of repeats for cross validation can be changed with the *nFolds* and *nRepeats* arguments. To perform 5-fold cross-validation of a Support Vector Machine with 2 repeats:

```{r}
result <- crossValidate(measurements, classes, classifier = "SVM", nFolds = 5, nRepeats = 2)
result
```
...
More traditionally, the distirubition of performance values of each complete cross-validation can be visualised by *performancePlot* by providing them as a list to the function. The default is to draw violin plot, but box plots could also be made. The default performance metric to plot is balanced error. If it's not already calculcated for all classifications, as in this case for DD, it will be done automatically.

```{r}
performancePlot(list(DMresults, DDresults))
```

We can observe that the spread of balanced error rates is small, but slightly wider for the differential distribution classifier.
