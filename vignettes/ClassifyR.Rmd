...
```{r}
result <- crossValidate(measurements, classes, classifier = "SVM", assayName = "RNA-seq",
...
mergePlot <- performancePlot(result, characteristicsList = list(x = "Assay Name"))
```
...
names(measurementsList) <- c("assay_1", "assay_1")
...
```{r}
accDistributionPlot <- performancePlot(list(DMresults, DDresults))
```
