...
DEresults <- runTests(ovarSet, validation = "bootstrap", resamples = 5, folds = 3,
                      params = list(SelectionParams(limmaSelection, nFeatures = c(25, 50, 75, seq(100, 1000, 100))),
                                    ...
                                    PredictParams(predict, TRUE, FALSE, function(result) result[["class"]])),
...
deviationResults <- runTests(ovarSet, validation = "bootstrap", resamples = 2, folds = 4,
                             params = list(SelectionParams(leveneSelection, nFeatures = c(25, 50, 75, seq(100, 1000, 100))),
                                           ...
                                           TrainParams(fisherDiscriminant, FALSE, TRUE),
                                           PredictParams(predictor = function(){}, FALSE, FALSE, function(result) result)),
                             verbose = 1)
deviationResults
```
...
distributionResults <- runTests(ovarSet, validation = "bootstrap", resamples = 2, folds = 2,
                                params = list(SelectionParams(KullbackLeiblerSelection,
                                                              ...
                                                              type = "dynamic"),
                                              TrainParams(naiveBayesKernel, FALSE, TRUE),
...
**Training Function** : The first argument must be a *matrix*. This is because most other R classifiers on CRAN take matrices. This avoids having to write interfaces for them. Other arguments may be anything. The argument *verbose* is sent from *runTest*, so it must handle it.  
