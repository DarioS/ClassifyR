...
To demonstrate some key features of ClassifyR, a data set consisting of the 2000 most variably expressed genes and 190 people will be used to quickly obtain results. The 20 patients which were treated by either nivolumab or pembrolizumab are divided into 11 responders and 9 non-responders. The journal article corresponding to the dataset was published in *Scientific Reports* in 2018 and is titled [A Nasal Brush-based Classifier of Asthma Identified by Machine Learning Analysis of Nasal RNA Sequence Data](http://www.nature.com/articles/s41598-018-27189-4). Although thousands of cells were measured for each sample, those measurements have been summarised to 1 value per protein by taking the median of all cells' measurements for a particular protein.
...
```{r, tidy = FALSE}
resubstituteParams <- ResubstituteParams(nFeatures = seq(5, 50, 5),
...
The error rate is about 20%. If only a vector of predictions and a vector of actual classes is available, such as from an old study which did not use **ClassifyR**, then *calcExternalPerformance* can be used on a pair of factor vectors which have the same length.
...
```{r}
rankOverlaps <- rankingPlot(list(DDresults), topRanked = 1:50,
                            xLabelPositions = c(1, seq(5, 50, 5)),
...
This ROC plot shows the classifiability of the asthma dataset is high. Other included functions which can output scores are *fisherDiscriminant*, *DLDApredictInterface*, and *SVMpredictInterface*.
